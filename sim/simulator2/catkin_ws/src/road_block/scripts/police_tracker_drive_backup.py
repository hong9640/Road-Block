...
from std_msgs.msg import String  # String 메시지 임포트
from scipy.interpolate import CubicSpline

class HDMapManager:
    # HDMapManager 클래스 로직은 변경하지 않습니다.
...
class AggressivePursuitNode:
    """
    1) ego-4이 추격자, ego-2가 표적  2) velocity는 발행 시 km/h로 변환
    """
    def __init__(self):
        ...
        self.lfd_gain = 1.0
        self.min_lfd, self.max_lfd = 5.0, 75.0
    ...
    def control_loop(self, event):
        ...
        if not self.chase_active:
            if self.pursuer_status and self.pursuer_status.velocity.x > 0.5: # 완전히 멈출 때까지 브레이크
                # longlCmdType=1은 accel/brake 제어 방식입니다.
