...
class IMGParser:
    def __init__(self, pkg_name='ssafy_3'):
        ...
        self.image_sub = rospy.Subscriber("/image_jpeg/compressed", CompressedImage, self.callback, queue_size=1)
        ...
        # 3차 다항, 차선폭 3.5 m, Y 마진 0.3 m, 예측 x 범위 10 m, 샘플 간격 0.1 m, 최소 점 5
        ...
        while not rospy.is_shutdown():
            if self.img_bgr is not None and self.is_status:
                ...
                img_lane_fit = self.draw_lane_img(
                    img_lane,
    ...
    def draw_lane_img(self, img, leftx, lefty, rightx, righty):
        ...
        leftx, lefty, rightx, righty : curve fitting 결과
        """
...
class BEVTransform:
    def __init__(self, params_cam, xb=10.0, zb=10.0):
        ...
        self.RT_b2g = np.matmul(
            np.matmul(self.traslationMtx(xb, 0, zb), self.rotationMtx(np.deg2rad(-90), 0, 0)),
    ...
    def warp_bev_img(self, img):
        return cv2.warpPerspective(img, self.perspective_tf, (self.width, self.height), flags=cv2.INTER_LINEAR)

    def warp_inv_img(self, img_warp):
        return cv2.warpPerspective(img_warp, self.perspective_inv_tf, (self.width, self.height), flags=cv2.INTER_LINEAR)
...
class CURVEFit:
    def __init__(self, order, alpha, lane_width, y_margin, x_range, dx, min_pts):
        ...
        # max_trials는 100으로 설정(필요시 200~500까지 올려 안정성↑)
        # sklearn 1.2+에서는 loss='absolute_error' 명칭이기도 하나, 여기선 'absolute_loss' 유지
    ...
    def preprocess_pts(self, lane_pts):
        ...
        mask_r = np.logical_and(y_g >= y_ransac_collect_r - self.y_margin,
                                y_g < y_ransac_collect_r + self.y_margin)
        y_right = y_g[mask_r]
        x_right = x_g[mask_r]

        y_ransac_collect_l = self.ransac_left.predict(X_g)
        mask_l = np.logical_and(y_g >= y_ransac_collect_l - self.y_margin,
                                y_g < y_ransac_collect_l + self.y_margin)
        y_left = y_g[mask_l]
        x_left = x_g[mask_l]
    ...
    def fit_curve(self, lane_pts):
        ...
        # 좌우 뒤바뀜 방지(간단한 오버랩 체크)
        if len(y_pred_l) == len(y_pred_r):
            try:
                idx3 = np.where(np.isclose(x_pred, 3.0))[0]
                if idx3.size > 0:
                    i3 = idx3[0]
                    if y_pred_r[i3] > 0:
                        y_pred_r = y_pred_l - self.lane_width
    ...
    def update_lane_width(self, y_pred_l, y_pred_r):
        self.lane_width = np.clip(np.max(y_pred_l - y_pred_r), 3.5, 5.0)
    ...
    def write_path_msg(self, x_pred, y_pred_l, y_pred_r, frame_id='/map'):
        ...
        self.lane_path.header.stamp = rospy.Time.now()
        ...
        for i in range(len(x_pred)):
            ...
            tmp_pose.header.stamp = self.lane_path.header.stamp
            tmp_pose.pose.position.x = float(global_result[0][0])
            tmp_pose.pose.position.y = float(global_result[1][0])
    ...
    def set_vehicle_status(self, vehicle_status):
        odom_quaternion = (
            vehicle_status.pose.pose.orientation.x,
            vehicle_status.pose.pose.orientation.y,
            vehicle_status.pose.pose.orientation.z,
            vehicle_status.pose.pose.orientation.w
        )
        _, _, vehicle_yaw = tf.transformations.euler_from_quaternion(odom_quaternion)
        self.vehicle_yaw = float(vehicle_yaw)
        self.vehicle_pos_x = float(vehicle_status.pose.pose.position.x)
        self.vehicle_pos_y = float(vehicle_status.pose.pose.position.y)
