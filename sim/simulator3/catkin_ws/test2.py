...
# ROS 서비스 및 메시지 타입 추가
from morai_msgs.srv import MoraiSyncModeCmdSrv, MoraiSyncModeSetGearSrv, MoraiWaitForTickSrv
...
class AggressivePursuitNode:
    ...
    def _change_ego2_gear(self):
        """
        Ego-2 차량의 기어를 'P'에서 'D'(Drive)로 변경하는 함수.
        ROS 서비스 목록에 맞춰 서비스 이름과 호출 방식을 수정.
        """
        
        # 현재 서비스 목록에 맞춰 서비스 이름 수정
        rospy.wait_for_service('/Ego-2/SyncModeCmd', timeout=5)
        rospy.wait_for_service('/Ego-2/SyncModeSetGear', timeout=5)
        rospy.wait_for_service('/Ego-2/SyncModeWaitForTick', timeout=5)
        
        # 서비스 프록시 생성 시 서비스 타입과 이름 일치
        sync_mode_cmd_proxy = rospy.ServiceProxy('/Ego-2/SyncModeCmd', MoraiSyncModeCmdSrv)
        sync_mode_set_gear_proxy = rospy.ServiceProxy('/Ego-2/SyncModeSetGear', MoraiSyncModeSetGearSrv)
        sync_mode_wait_for_tick_proxy = rospy.ServiceProxy('/Ego-2/SyncModeWaitForTick', MoraiWaitForTickSrv)
        
        try:
            rospy.loginfo("Ego-2 차량 기어를 변경합니다...")
            
            # 1. 동기화 모드 시작: user_id 필드 제거
            sync_cmd_res = sync_mode_cmd_proxy(start_sync_mode=True, time_step=20)
            if not sync_cmd_res.success:
                rospy.logerr("Failed to start sync mode: %s", sync_cmd_res.message)
                return
            rospy.loginfo("동기화 모드 시작 완료.")
            
            # 2. 현재 프레임 정보 가져오기: /Ego-2/SyncModeInfo 토픽 사용
            rospy.loginfo("프레임 정보 대기 중...")
            sync_info = rospy.wait_for_message('/Ego-2/SyncModeInfo', SyncModeInfo, timeout=10)
            current_frame = sync_info.frame
            
            # 3. 기어를 'D'(4)로 설정
            rospy.loginfo("기어를 D(4)로 설정합니다.")
            set_gear_res = sync_mode_set_gear_proxy(is_set_gear=True, gear=4, frame=current_frame)
            if not set_gear_res.success:
                rospy.logerr("Failed to set gear: %s", set_gear_res.message)
                return
            
            # 4. 1틱 대기 (커밋용)
            rospy.loginfo("1틱 대기 중...")
            wait_for_tick_res = sync_mode_wait_for_tick_proxy(user_id='ssafy_ad_007', frame=current_frame, command=True)
            if not wait_for_tick_res.success:
                rospy.logerr("Failed to wait for tick: %s", wait_for_tick_res.message)
                return
            
            # 5. 동기화 모드 종료: user_id 필드 제거
            rospy.loginfo("동기화 모드 종료.")
            sync_cmd_res = sync_mode_cmd_proxy(start_sync_mode=False, time_step=0)
            if not sync_cmd_res.success:
                rospy.logerr("Failed to stop sync mode: %s", sync_cmd_res.message)
                return
            
            rospy.loginfo("[완료] Ego-2 차량의 기어가 성공적으로 D(4)로 변경되었습니다.")
            
        except rospy.ServiceException as e:
            rospy.logerr("ROS 서비스 호출 실패: %s", e)
        except rospy.ROSException as e:
            rospy.logerr("ROS 에러: %s", e)
        except Exception as e:
            rospy.logerr("예상치 못한 에러 발생: %s", e)
