...
class LaneFollowerBEVStable:
    ...
    def bev_image_callback(self, msg):
        ...
        # 노란색 차선 HSV 범위
        # Hue는 노란색 계열인 15~45 사이를 포함하도록 확장
        # Saturation과 Value 범위를 조금 더 관대하게 설정
        lower_ylane = np.array([15, 70, 100])
        upper_ylane = np.array([45, 255, 255])

        img_wlane = cv2.inRange(img_hsv, lower_wlane, upper_wlane)
        img_ylane = cv2.inRange(img_hsv, lower_ylane, upper_ylane)
        
        # 명도(V) 채널을 추가로 활용하여 노이즈 제거
        # 명도가 일정 수준 이상인 픽셀만 선택하여 더 정확한 차선 검출
        _, img_v_binary = cv2.threshold(V, 200, 255, cv2.THRESH_BINARY)
        
        # 노란색 차선 이미지에 명도 필터링을 적용하여 노이즈 감소
        img_ylane = cv2.bitwise_and(img_ylane, img_v_binary)
        
        # 흰색 차선과 노란색 차선 이진화 결과를 합치기
        lane_binary = cv2.bitwise_or(img_wlane, img_ylane)

        # 2. 모폴로지 연산으로 노이즈 제거 및 차선 연결
        ...
        if detected_left or detected_right:
            ...
            try:
                ...
                if self.left_fit is not None and self.right_fit is not None:
                    ...
                    # 왼쪽 차선만 감지된 경우, 표준 차선 너비를 사용하여 중앙선 추정
                    ...
                    # 오른쪽 차선만 감지된 경우, 표준 차선 너비를 사용하여 중앙선 추정
