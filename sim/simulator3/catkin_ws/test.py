...
class HDMapManager:
    ...
    def get_dp_global_path(self, start_proj, end_proj, reverse_penalty=1.0):
        start_link_id, end_link_id = start_proj['link_id'], end_proj['link_id']
        
        # 비용 및 경로 저장 딕셔너리 초기화
        min_cost = {link_id: float('inf') for link_id in self.links_data}
        path_sequence = {}
        
        # 출발점 초기화
        min_cost[start_link_id] = 0
        link_order = list(self.links_data.keys())
        
        # 동적 계획법: 모든 링크를 순회하며 최단 비용 계산
        for _ in range(len(self.links_data)):
            for link_id in link_order:
                if min_cost[link_id] == float('inf'):
                    continue
                
                current_link = self.links_data[link_id]
                to_node_id = current_link['to_node_idx']
                from_node_id = current_link['from_node_idx']
                
                # 순방향 인접 링크 탐색
                for neighbor_id, neighbor_link in self.links_data.items():
                    if neighbor_link['from_node_idx'] == to_node_id:
                        cost = current_link['link_length']
                        if min_cost[link_id] + cost < min_cost[neighbor_id]:
                            min_cost[neighbor_id] = min_cost[link_id] + cost
                            path_sequence[neighbor_id] = (link_id, 'forward')
                    
                    # 역방향 인접 링크 탐색
                    if neighbor_link['to_node_idx'] == to_node_id:
                        cost = current_link['link_length'] * reverse_penalty
                        if min_cost[link_id] + cost < min_cost[neighbor_id]:
                            min_cost[neighbor_id] = min_cost[link_id] + cost
                            path_sequence[neighbor_id] = (link_id, 'reverse')

        # 경로 재구성
        if min_cost[end_link_id] == float('inf'):
            return None
            
        path = []
        current_id = end_link_id
        while current_id in path_sequence:
            prev_id, direction = path_sequence[current_id]
            path.insert(0, (current_id, direction))
            current_id = prev_id
        path.insert(0, (start_link_id, 'forward'))
...
class AggressivePursuitNode:
    def __init__(self):
        ...
        self.ctrl_pub = rospy.Publisher('/Ego-2/ctrl_cmd', CtrlCmd, queue_size=1)
        self.evasive_path_pub = rospy.Publisher('/evasive_path', Path, queue_size=1)
    ...
    def control_loop(self, event):
        ...
        if self.global_path is None or self._should_update_global_path(goal_proj):
            ...
            path_seq = self.map_manager.get_dp_global_path(ego_proj, goal_proj, self.REVERSE_DRIVING_PENALTY)
            if path_seq:
        ...
        path_to_follow = self.local_path
        steering_cmd = self.calculate_steering(path_to_follow)
    ...
    def calculate_steering(self, path):
