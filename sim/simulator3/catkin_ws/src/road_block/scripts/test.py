...
class HDMapManager:
    ...
    def get_global_path(self, start_proj, end_proj, reverse_penalty=1.0):
        ...
        while open_set:
            ...
            # 역방향 진입 시 reverse_penalty를 적용합니다.
...
class AggressivePursuitNode:
    ...
    def __init__(self):
        ...
        # [수정] 역방향 진입 패널티를 높여 A*가 역주행 링크를 선택하는 것을 방지
        self.REVERSE_DRIVING_PENALTY = 100.0
        ...
        # 고속 안정성을 위한 튜닝 파라미터
        self.lfd_gain = 1.2                  # 속도 대비 전방 주시 거리 게인
        self.min_lfd, self.max_lfd = 4.0, 60.0 
        self.VELOCITY_KP = 1.0               
        self.MAX_VELOCITY_KPH = 250.0         
        
        # 공격적 회전 로직을 위한 파라미터
        # [수정] 급격한 조향 시작 거리를 줄여 안전성 확보
        self.AGGRESSIVE_TURN_DISTANCE = 5.0
        ...
        rospy.loginfo("공격적 최단경로 추격 노드 (자동 시작, 안전성 강화) 시작.")
    ...
    def calculate_steering(self, driving_mode='forward'):
        ...
        for pose in self.local_path.poses:
            ...
            if is_lookahead_point:
                ...
                # AGGRESSIVE_TURN_DISTANCE가 5.0으로 줄어들어 급격한 조향이 시작되는 지점 지연
