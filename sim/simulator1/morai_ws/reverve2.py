...
class AggressivePursuitNode:
    ...
    def __init__(self):
        ...
        rospy.Subscriber("/Ego_topic",       EgoVehicleStatus, self.pursuer_status_callback)
        ...
        self.GLOBAL_PATH_UPDATE_THRESHOLD_DIST = 15.0
        ...
        self.MAX_CURVATURE_VELOCITY = 40.0 # m/s
        self.CURVATURE_LOOKAHEAD = 10.0 # 곡률 계산을 위한 로컬 경로 거리
    ...
    def calculate_velocity(self, pursuer_status, goal_proj):
        # 코너 진입 시 속도 제어를 위한 곡률 기반 속도 계산 로직
        current_v_mps = pursuer_status.velocity.x

        if self.local_path and len(self.local_path.poses) > 2:
            # 로컬 경로의 곡률을 계산
            p1 = np.array([self.local_path.poses[0].pose.position.x, self.local_path.poses[0].pose.position.y])
            p2 = np.array([self.local_path.poses[1].pose.position.x, self.local_path.poses[1].pose.position.y])
            p3_idx = min(len(self.local_path.poses) - 1, int(self.CURVATURE_LOOKAHEAD / np.linalg.norm(p2 - p1)))
            p3 = np.array([self.local_path.poses[p3_idx].pose.position.x, self.local_path.poses[p3_idx].pose.position.y])

            # 3점 기반 곡률 계산
            p1_vec = p2 - p1
            p2_vec = p3 - p2

            cross_product = np.cross(p1_vec, p2_vec)
            denominator = np.linalg.norm(p1_vec) * np.linalg.norm(p2_vec) * np.linalg.norm(p1_vec - p2_vec)

            if abs(denominator) > 1e-6:
                curvature = (2 * cross_product) / denominator
            else:
                curvature = 0

            max_curve_v_mps = math.sqrt(1.0 / (abs(curvature) + 1e-6))

            # 곡률 기반 속도 제한
            velocity_limit_mps = min(max_curve_v_mps, self.MAX_CURVATURE_VELOCITY / 3.6)

        else:
            velocity_limit_mps = self.MAX_CURVATURE_VELOCITY / 3.6
        ...
        # 최종 속도는 기존 목표 속도와 곡률 제한 속도 중 더 작은 값
        final_target_v_mps = min(target_v, velocity_limit_mps)
